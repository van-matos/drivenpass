...
export async function createCard(card: ICardData) {
    ...
    card.password = encryptionUtils.encrypt(card.password);
    card.securityCode = encryptionUtils.encrypt(card.securityCode);
...
export async function getCards(userId: number) {
    ...
    cards.forEach((card: { password: string; securityCode: string; }) => {
        card.password = encryptionUtils.decrypt(card.password);
        card.securityCode = encryptionUtils.decrypt(card.securityCode);
    });
